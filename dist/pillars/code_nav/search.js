...
export function searchSymbols(db, query, limit = 20) {
    const q = query.trim();
    if (!q)
        return [];
    const qLc = q.toLowerCase();
    // Channel 1: SQL LIKE — exact name first, then substring name, then substring content
    const exact = db.prepare("SELECT rowid FROM symbols WHERE name_lc = ? LIMIT 50").all(qLc);
    const prefix = db.prepare("SELECT rowid FROM symbols WHERE name_lc LIKE ? AND name_lc != ? LIMIT 50").all(`${qLc}%`, qLc);
    const substr = db.prepare("SELECT rowid FROM symbols WHERE name_lc LIKE ? AND name_lc NOT LIKE ? LIMIT 50").all(`%${qLc}%`, `${qLc}%`);
    const contentSub = db.prepare("SELECT rowid FROM symbols WHERE content_lc LIKE ? LIMIT 50").all(`%${qLc}%`);
    const sqlRanked = [...exact, ...prefix, ...substr, ...contentSub];
    // Channel 2: Trigram fuzzy
    const qTri = trigrams(q);
    const candidates = db.prepare("SELECT rowid, name_lc FROM symbols LIMIT ?").all(TRIGRAM_CANDIDATES);
    ...
    const rows = db.prepare(`SELECT rowid, id, path, start_line, end_line, kind, name, content FROM symbols WHERE rowid IN (${placeholders})`).all(...ids);
